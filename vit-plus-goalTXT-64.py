...
def my_main(cfg: DictConfig):
    ...
    import pandas as pd
    action_labels = pd.qcut(np.reshape(dataset_tmp["action"][:,:1], (dataset_tmp["action"].shape[0])), cfg.action_bins, labels=False)
    ...
    dataset_tmp = {
        ...
        "action": torch.tensor(np.reshape(action_labels, (dataset_tmp["action"].shape[0], 1)), dtype=torch.uint8).to(device),            
