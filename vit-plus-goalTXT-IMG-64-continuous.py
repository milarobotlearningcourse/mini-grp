...
def get_batch_vit(split, dataset, batch_size):
    ...
    x_goal_img = torch.tensor(data["img"][ix], dtype=torch.float)
...
class GRP(nn.Module):
  def __init__(self, dataset, cfg, mlp_ratio=4):
    ...
    self.register_buffer('positional_embeddings', calc_positional_embeddings(1 + self._cfg.n_patches ** 2 + self._cfg.n_patches ** 2 + self._cfg.block_size, cfg.n_embd), persistent=False)
