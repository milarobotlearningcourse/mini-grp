...
def get_batch_vit(split, dataset, batch_size):
    ...
    x_goal_txt = torch.tensor(data["goal"][ix], dtype=torch.long)
    y = torch.tensor(data["action"][ix], dtype=torch.long)
    # x, y = x.to(device), y.to(device)
    return x, x_goal, x_goal_txt, y
...
def estimate_loss(model):
    ...
    for split in ['train', 'val']:
        ...
        for k in range(model._cfg.eval_iters):
            X, x_goal, x_goal_txt, Y = get_batch_vit(split, model._dataset, model._cfg.batch_size)
            logits, loss = model(X, x_goal, x_goal_txt, Y)
            losses[k] = loss.item()
...
class GRP(nn.Module):
  def __init__(self, dataset, cfg, mlp_ratio=4):
    ...
    self.register_buffer('positional_embeddings', calc_positional_embeddings(1 + cfg.n_patches ** 2 + cfg.n_patches ** 2 + self._cfg.block_size, cfg.n_embd), persistent=False)
  ...
  def forward(self, images, x_goal, goals, targets=None):
    ...
    out = torch.cat((self.class_tokens.expand(n, 1, -1), out, out_g, goals_e), dim=1)
...
def my_main(cfg: DictConfig):
    ...
    dataset_tmp = {
        ...
        "action": np.concatenate((np.array(dataset["action"]), 
                                ...
                                # np.array(dataset["open_gripper"]) + np.random.normal(0.0, 0.01, size = (len(dataset["open_gripper"]), 1))
        ), axis=1),
    ...
    decode_action = lambda binN: action_bins[binN] # Undo mapping to [-1, 1]
